...
function AppDashboard({
  ...
  function handleDownloadCalendar(plan: AppSavedPlan, request?: WeeklyRequest) {
    try {
      const planScheduleEvents = activeScheduleEvents.filter((event) =>
        isScheduleEventInRange(event, plan.week_start_date, plan.week_end_date),
      );
      const calendarText = buildCalendarFile(
        plan,
        request,
        planScheduleEvents,
      );
...
function buildCalendarFile(
  ...
  const events = dateList.map((day, index) => {
    ...
    const dayScheduleEvents = scheduleEvents.filter(
      (event) => event.event_date === day.date,
    );
    const scheduleEventSummary =
      dayScheduleEvents.length > 0
        ? `Known schedule context:\n${dayScheduleEvents
            ...
            .map(
              (event) =>
                `- ${formatScheduleEventTiming(event)} | ${event.category} | ${event.title}`,
            )
            .join("\n")}`
        : "";
